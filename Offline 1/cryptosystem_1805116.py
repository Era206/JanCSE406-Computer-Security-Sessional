...
def read_text_files(file1_path, file2_path):
...
# in other word g(w[4]) paite ja kortam
...
# word adding, w[4] paite w[0]er sathe g(w[3]) add, bakigula just aagertar sathe xor 
...
# key size er basis e round key matrix banay dibe, 128 bit er jnno 44 ta word,192 r jnno 52, 256 er jnno 60 word
...
# -----------------secret key ar plain text theke matrix paite-----------------
...
# -----------encryption, decryption er functions ekhan theke start --------------
# add duitatei same, svalue ar rounded matrix duitar alada
...
def get_rounded_matrix(matrix1):
    result = [[None] * len(matrix1[0]) for _ in range(len(matrix1))]
    for i in range(len(matrix1)):
        for j in range(len(matrix1[0])):
            value=(j+i)%4
...
# mix column duitar e same, just encryption e mixer, decryption e invMixer send kora lagbe
...
# important, chunk er kaj ekhan theke start,ei function diya text_size er boro input ke alada kore
...
# ---------------encryption er function-----------------------
def cipher_text_generation(text, round_key_matrix,key_size):
    cipher_text=[]
    if key_size==128:
        round=11
        last_round=10
    ...
    # round 0 er cipher count, sobar jnno same
    cipher_text=text_to_bitvector_matrix(text,4,4)
    ...
    cipher_text=add_matrices(cipher_text, transposed_key_matrix)

    for i in range(1,round-1):
        substituted_matrix=get_svalue_matrix(cipher_text)
        rounded_matrix=get_rounded_matrix(substituted_matrix)
        # rounded_BitVector_matrix=hex_to_BitVector_matrix(rounded_matrix)
        mix_column=mix_column_matrix(Mixer, rounded_matrix)
        transposed_key_matrix=round_key_matrix[i*4:(i*4) + 4]
        transposed_key_matrix = [[transposed_key_matrix[j][i] for j in range(len(transposed_key_matrix))] for i in range(len(transposed_key_matrix[0]))]
        cipher_text=add_matrices(mix_column,transposed_key_matrix)
       
    substituted_matrix=get_svalue_matrix(cipher_text)
    rounded_matrix=get_rounded_matrix(substituted_matrix)
    transposed_key_matrix=round_key_matrix[last_round*4:(last_round*4) + 4]
    transposed_key_matrix = [[transposed_key_matrix[j][i] for j in range(len(transposed_key_matrix))] for i in range(len(transposed_key_matrix[0]))]
    cipher_text=add_matrices(rounded_matrix,transposed_key_matrix)
    cipher_text=[[cipher_text[j][i] for j in range(len(cipher_text))] for i in range(len(cipher_text[0]))]


    # for row in cipher_text:
    #     for bitvector in row:
    #         print(hex(ord(chr(int(bitvector)))), end=' ')

    return cipher_text

# --------------------encryption----------------------------------------
def aes_encryption(plain_text, secret_key, text_chars, key_size, round_key_matrix):
    text_matrix=dividing_text(plain_text, text_chars)
    text_nums=len(text_matrix)
    # round_key_matrix=get_round_key_matrix(secret_key,key_size)
    cipher_text_matrix=[]
    for i in range(text_nums):
        cipher_text_matrix.extend(cipher_text_generation(text_matrix[i], round_key_matrix, key_size))

    # print(cipher_text_matrix)
    # char_matrix = [[chr(int(bit_vector.get_bitvector_in_hex(), 16)) for bit_vector in row] for row in cipher_text_matrix]
    # for row in cipher_text_matrix:
    #     for bitvector in row:
    #         print(hex(ord(chr(int(bitvector)))), end='\n')
...
# -----------------------decryption----------------------------------------------------
def aes_decryption(cipher_text, secret_key, text_chars, key_size, round_key_matrix):
    ...
    # char_matrix = [[chr(int(bit_vector.get_bitvector_in_hex(), 16)) for bit_vector in row] for row in plain_text_matrix]
...
def ascii_to_hex_matrix(ascii_matrix):
    hex_matrix = []

    for row in ascii_matrix:
        hex_row = [hex(char)[2:] for char in row]
...
file1_path = "text_1805116.txt"
file2_path = "key_1805116.txt"
plainText, secretKey = read_text_files(file1_path, file2_path)

# -----------------key generation-------------------------------------------------
key_size=128
t1=time.perf_counter_ns()
round_key_matrix=get_round_key_matrix(secretKey,key_size)
t2=time.perf_counter_ns()
key_scheduling_time=(t2-t1)/1000000000
# print(key_scheduling_time)

# # -----------------encryption part ---------------------------------------------
text_chars=16
t1=time.perf_counter_ns()
encrypted_BitVector_Matrix=aes_encryption(plainText, secretKey, text_chars, key_size, round_key_matrix)
t2=time.perf_counter_ns()
encryption_time=(t2-t1)/1000000000
encrypted_hex_matrix=BitVector_to_hexadecimal_matrix(encrypted_BitVector_Matrix)
encrypted_char_matrix=BitVector_to_character_matrix(encrypted_BitVector_Matrix)
encrypted_char_string = ''.join([''.join(row) for row in encrypted_char_matrix])
# print(encrypted_char_string)


# -------------------decryption part-------------------------------------------
text_chars=16
t1=time.perf_counter_ns()
decrypted_BitVector_Matrix=aes_decryption(encrypted_char_string,secretKey,text_chars,key_size, round_key_matrix)
t2=time.perf_counter_ns()
decryption_time=(t2-t1)/1000000000
decrypted_hex_matrix=BitVector_to_hexadecimal_matrix(decrypted_BitVector_Matrix)
...
# print(decrypted_char_string)


# -----------------printing section-----------------------------------------
print("Plain Text:")
print("In ASCII: " + plainText)
hex_string= binascii.hexlify(plainText.encode()).decode()
print("In Hex: "+ hex_string)
print(' ')

print("Key:")
print("In ASCII: " + secretKey)
hex_string = binascii.hexlify(secretKey.encode()).decode()
print("In Hex: "+ hex_string)
print(' ')

print("Cipher Text:")
hex_string = ''.join(''.join(row) for row in encrypted_hex_matrix)
print("In Hex: "+ hex_string)
print("In ASCII: " + encrypted_char_string)


print(' ')

print("Deciphered Text:")
hex_string = ''.join(''.join(row) for row in decrypted_hex_matrix)
print("In Hex: "+ hex_string)
print("In ASCII: " + decrypted_char_string)

print(' ')

print("Execution time details:")
print("Key Scheduling: "+ str(key_scheduling_time) + " seconds")
